   ...
   var radial = function (canvas, parameters) {
    ...
    var gaugeType = (undefined === parameters.gaugeType ? steelseries.GaugeType.TYPE4 : parameters.gaugeType),
        ...
        alarmLoop = (undefined === parameters.alarmLoop ? false : parameters.alarmLoop);
        ...
        if(alarmLoop) {
            audioElement.setAttribute('loop', alarmLoop);
        }
        var steelEvent;
        ...
        var ledBufferOn = steelseries.createBuffer(ledSize, ledSize);
        var ledContextOn = ledBufferOn.getContext('2d');

        // Buffer for led off painting code
        var ledBufferOff = steelseries.createBuffer(ledSize, ledSize);
        ...
        var userLedBufferOn = steelseries.createBuffer(ledSize, ledSize);
        var userLedContextOn = userLedBufferOn.getContext('2d');

        // Buffer for user led off painting code
        var userLedBufferOff = steelseries.createBuffer(ledSize, ledSize);
        ...
        var minMeasuredValueBuffer = steelseries.createBuffer(Math.ceil(size * 0.028037), Math.ceil(size * 0.028037));
        var minMeasuredValueCtx = minMeasuredValueBuffer.getContext('2d');

        // Buffer for the maxMeasuredValue indicator
        var maxMeasuredValueBuffer = steelseries.createBuffer(Math.ceil(size * 0.028037), Math.ceil(size * 0.028037));
        var maxMeasuredValueCtx = maxMeasuredValueBuffer.getContext('2d');

        // Buffer for pointer image painting code
        var pointerBuffer = steelseries.createBuffer(size, size);
        ...
        if (useOdometer && lcdVisible) {
            odoBuffer = steelseries.createBuffer(10, 10);          // size doesn't matter, it will get reset by odometer code
...
// ******************** private methods ************************************         
        ...
        var drawPostsImage = function (ctx) {
            ...
            if ('type1' === gaugeType.type) {
                ...
                ctx.drawImage(steelseries.createKnobImage(Math.ceil(imageHeight * 0.037383), steelseries.KnobType.STANDARD_KNOB, knobStyle), imageWidth * 0.523364, imageHeight * 0.130841);
            }

            if ('type1' === gaugeType.type || 'type2' === gaugeType.type) {
                // Draw min left post
                ctx.drawImage(steelseries.createKnobImage(Math.ceil(imageHeight * 0.037383), steelseries.KnobType.STANDARD_KNOB, knobStyle), imageWidth * 0.130841, imageHeight * 0.514018);
            }

            if ('type2' === gaugeType.type || 'type3' === gaugeType.type) {
                // Draw max right post
                ctx.drawImage(steelseries.createKnobImage(Math.ceil(imageHeight * 0.037383), steelseries.KnobType.STANDARD_KNOB, knobStyle), imageWidth * 0.831775, imageHeight * 0.514018);
            }

            if ('type3' === gaugeType.type) {
                // Draw min center bottom post
                ctx.drawImage(steelseries.createKnobImage(Math.ceil(imageHeight * 0.037383), steelseries.KnobType.STANDARD_KNOB, knobStyle), imageWidth * 0.523364, imageHeight * 0.831775);
            }

            if ('type4' === gaugeType.type) {
                // Min post
                ctx.drawImage(steelseries.createKnobImage(Math.ceil(imageHeight * 0.037383), steelseries.KnobType.STANDARD_KNOB, knobStyle), imageWidth * 0.336448, imageHeight * 0.803738);

                // Max post
                ctx.drawImage(steelseries.createKnobImage(Math.ceil(imageHeight * 0.037383), steelseries.KnobType.STANDARD_KNOB, knobStyle), imageWidth * 0.626168, imageHeight * 0.803738);
            }
        ...
        var drawTickmarksImage = function (ctx, labelNumberFormat) {
            ...
            for (i = minValue; parseFloat(i.toFixed(2)) <= MAX_VALUE_ROUNDED; i += minorTickSpacing) {
                ...
                if (majorTickCounter === maxNoOfMinorTicks) {
                    ...
                    switch (tickLabelOrientation.toLowerCase()) {
                    case 'horizontal':
        ...
        var init = function (parameters) {
            ...
            if (drawLed) {
                ...
                ledContextOn.drawImage(steelseries.createLedImage(Math.ceil(size * 0.093457), 1, ledColor), 0, 0);

                // Draw LED OFF in ledBuffer_OFF
                ledContextOff.drawImage(steelseries.createLedImage(Math.ceil(size * 0.093457), 0, ledColor), 0, 0);
            }

            if (drawUserLed) {
                // Draw user LED ON in userLedBuffer_ON
                userLedContextOn.drawImage(steelseries.createLedImage(Math.ceil(size * 0.093457), 1, userLedColor), 0, 0);

                // Draw user LED OFF in userLedBuffer_OFF
                userLedContextOff.drawImage(steelseries.createLedImage(Math.ceil(size * 0.093457), 0, userLedColor), 0, 0);
            }

            // Draw min measured value indicator in minMeasuredValueBuffer
            if (minMeasuredValueVisible) {
                minMeasuredValueCtx.drawImage(steelseries.createMeasuredValueImage(Math.ceil(size * 0.028037), steelseries.ColorDef.BLUE.dark.getRgbaColor(), true, true), 0, 0);
            }

            // Draw max measured value indicator in maxMeasuredValueBuffer
            if (maxMeasuredValueVisible) {
                maxMeasuredValueCtx.drawImage(steelseries.createMeasuredValueImage(Math.ceil(size * 0.028037), steelseries.ColorDef.RED.medium.getRgbaColor(), true), 0, 0);
            }
            ...
            if (drawPointer) {
                steelseries.drawPointerImage(pointerContext, imageWidth, pointerType, pointerColor, backgroundColor.labelColor);
            }
            ...
            if (drawForeground && foregroundVisible) {
                ...
                steelseries.drawRadialForegroundImage(foregroundContext, foregroundType, imageWidth, imageHeight, knobVisible, knobType, knobStyle, gaugeType);
            } 
        ...
        var toggleAndRepaintUserLed = function () {
            if (userLedVisible) {
                ...
                if (!repainting) {
                    repainting = true;
                    steelseries.requestAnimFrame(self.repaint);
                }
            }
        };
    ...
    var requestAnimFrame = (function () {
        ...
        //************************************ Public methods **************************************
        this.getGaugeType = function() {
            return gaugeType.type.toUpperCase()
        };

        this.setGaugeType = function(type) {
            type=type.toUpperCase()
            if(type !== this.getGaugeType()) { 
             gaugeType = steelseries.GaugeType[type]
             this.repaint()   ;
            } 
             return this
         };
         this.getTickOrientation = function() {
            return tickLabelOrientation
         }
         this.setTickOrientation = function(tickOrientation) {
            tickLabelOrientation = tickOrientation
            this.repaint();
            return this
        };
        ...
        this.setTickFormat = function(format) {
            labelNumberFormat = steelseries.LabelNumberFormat[format]
            this.repaint()
            return this;
        }
        this.setNiceScale = function(nice) {
            niceScale = nice
            this.repaint()
            return this
        }
        ...
        this.setValue = function (newValue) {
            ...
            if (value !== targetValue) {
                value = targetValue;
                self.setOdoValue(value)

                if (value > maxMeasuredValue) {
                    maxMeasuredValue = value;
                    if(maxMeasuredValueVisible) {
                        //triggerCustomEvent({name:'measured',msg:'changed',value:value})
                    }
                }
                ...
                if ((value >= threshold && !ledBlinking && thresholdRising) ||
                   ...
                   //if(thresholdVisible) { triggerCustomEvent({name:'threshold',msg:'reached',value:value}) }
                    ...
                    // if(thresholdVisible) { triggerCustomEvent({name:'threshold',msg:'normal',value:value}) }
        ...
        this.setValueAnimated = function (newValue, callback) {
            ...
            if (value !== targetValue) {
                ...
                var maxMeasuredValChanged = false
                var minMeasuredValChanged = false
                tween.onMotionChanged = function (event) {
                    ...
                    if (value > maxMeasuredValue) {
                        maxMeasuredValue = value;
                        maxMeasuredValChanged = true
                           
                    } else {  maxMeasuredValChanged = false}

                    if (value < minMeasuredValue) {
                        minMeasuredValue = value;  
                        minMeasuredValChanged = true
                    }
                    if (!repainting) {
                        repainting = true;
                        requestAnimFrame(self.repaint);
                ...
                if (callback && typeof(callback) === "function") {
                   ...
                   tween.onMotionFinished = function() {
                        callback({
                                ...
                                maxMeasuredValChanged:maxMeasuredValChanged,
                                minMeasuredValChanged:minMeasuredValChanged

                            });
                    }
                    
                }
                tween.onMotionStopped = function() {
                  //  console.log("%c Motion stopped","color:brown")
                }
